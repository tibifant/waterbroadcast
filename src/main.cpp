...
const char* wateringTimeSubscriberTopic = "watering_time1";
...
const char* wateringTimeSubscriberTopic = "watering_time2";
#else
...
void callback(char* topic, byte* payload, unsigned int length) {  
  ...
  } else if (strcmp(topic, wateringTimeSubscriberTopic) == 0) {
    if (length >= 1 && *payload > 0 && *payload < 6) {
      wateringTime = *payload * 1000;      
    }
  }
}
...
void taskPump(void *parameter) {
  while (true) {
    ...
    if (pumpShouldTrigger) {
      ...
      vTaskDelay(wateringTime / portTICK_PERIOD_MS);
  ...
  while (true) {
    while (!client.connected()) {
      ...
      if (client.connect(clientName)) {
        ...
        client.subscribe(wateringTimeSubscriberTopic);
      } else {
